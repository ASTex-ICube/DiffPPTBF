...
float procedural_pptbf(float xx, float yy, float tx, float ty, float rescalex, float rescaley, float zoom,
  ...
  //float y = 100.0 + (-ppx * sin(-alpha) + ppy * cos(-alpha)) / rescaley * zoom;
...
__kernel void pptbf(const uint size, float tx, float ty, float rescalex, float rescaley, float zoom,
  ...
  */
  ...
  image_g[i * size + j] =
      procedural_pptbf(x, y, tx, ty, rescalex, rescaley, zoom, alpha, tt, footprint, jitter, arity, ismooth,
