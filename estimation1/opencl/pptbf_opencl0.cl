...
// Ecrire une fonction qui fournit les valeurs aléatoires pour
// les noyaux de Gabor qui s'applique aux points

/*
void compute_gabor_values() {
...
__kernel void gabor(__global float *p,
                    __global __write_only float *gabor_values
                    ) {
  ...
  float gb = next();
  float gc = next();
  gabor_values[3 * k + 0] = ga;
  gabor_values[3 * k + 1] = gb;
  gabor_values[3 * k + 2] = gc;
}
...
float procedural_pptbf(float xx, float yy, float tx, float ty, float zoom,
  ...
  float xx0 = xx + amp * cnoise2DG(xx * zoom * 0.5f + rx, yy * zoom * 0.5f) +
        amp * 0.5f * cnoise2DG(xx * zoom + rx, yy * zoom) +
        amp * 0.25f * cnoise2DG(xx * zoom * 2.0f + rx, yy * zoom * 2.0f) +
        amp * 0.125f * cnoise2DG(xx * zoom * 4.0f + rx, yy * zoom * 4.0f);

  float yy0 = yy + amp * cnoise2DG(xx * zoom * 0.5f, yy * zoom * 0.5f + ry) +
        amp * 0.5f * cnoise2DG(xx * zoom, yy * zoom + ry) +
        amp * 0.25f * cnoise2DG(xx * zoom * 2.0f, yy * zoom * 2.0f + ry) +
        amp * 0.125f * cnoise2DG(xx * zoom * 4.0f, yy * zoom * 4.0f + ry);

	xx0 = xx0 - 0.5f;
	yy0 = yy0 - 0.5f;

  float theta = (1.0f - theta_s) * (-0.1f * M_PI) + theta_s * (0.1f * M_PI);
	float zetax = (1.0f - zetax_s) * 0.4f + zetax_s * 1.0f;
	float zetay = (1.0f - zetay_s) * 0.4f + zetay_s * 1.0f;
