...
from opencl.pptbf_opencl import cl_pptbf, cl_pptbf_0
...
def generate_new_image(count):
    ...
    p_np, npp_np, fbm_np = cl_pptbf_0(size, tx, ty, zoom, alpha, tiling)
    nb_points = p_np.shape[0]

    points.append(nb_points)
...
print("MIN NUMBER OF POINTS: " + str(np.min(points)))
print("MAX NUMBER OF POINTS: " + str(np.max(points)))
...
trainPointY = points[:trainNum]
testPointY = points[-testNum:]
...
del images, tilings, points, jitters, normblends, wsmooths, winfeats, anisos, sigcoss, deltas, larps

dataset = tf.data.Dataset.from_tensor_slices((trainX, {"tiling_output": trainTilingY, "jittering_output": trainJitterY, 
                                                        "point_output": trainPointY,
...
with mirrored_strategy.scope():
    ...
    [
        keras.layers.experimental.preprocessing.Resizing(size, size),
    ...
    outputsPoint = keras.layers.Dense(1, activation='linear', name='point_output')(bottleneck)
    ...
    model = keras.Model(inputs, outputs=[outputsTiling, outputsJittering, outputsPoint, outputsNorm, outputsWsmooth, outputsWinfeat, outputsAniso, outputsSigcos, outputsDelta, outputsLarp])
    model.summary()
    ...
    model.compile(
        ...
        'point_output' : 'mean_squared_error', 
        ...
        metrics={'tiling_output': 'accuracy'}
    )
...
checkpoint = ModelCheckpoint('complete_withPoints-{val_loss:.3f}.h5', save_best_only=True, monitor='val_loss', mode='min', verbose=1)
