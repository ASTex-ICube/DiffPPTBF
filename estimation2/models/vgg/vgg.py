...
class VGG(nn.Module):
    ...
    def _conv_block(self, x, features, num_layers, block_num, act, dtype='float32'):
        for l in range(num_layers):
            ...
            """
            # ADDED BY GUILLAUME BALDI:
            # - select the last layer of each block: -1
            # - select the first layer of each block: 0
            #if self.num_layers == 5 and l == 0:
            if self.num_layers == 5 and l == num_layers - 1:
                act[layer_name] = x
            if self.num_layers == 12 and block_num != 5:
                act[layer_name] = x
            x = nn.relu(x)
            #act[f'relu{block_num}_{l + 1}'] = x
            """
            
            # For use with all layers in the selected blocks
            act[layer_name] = x
            x = nn.relu(x)
