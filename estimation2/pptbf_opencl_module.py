...
kernelFile = open("pptbf_opencl.cl", "r")
...
# Generation of feature function parameters
# (anisotropic Gaussian function)
def cl_agauss(p):

	p_g = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=p)
	
	agauss_values = np.zeros((p.shape[0], 3), dtype=np.float32)
	agauss_values_g = cl.Buffer(ctx, mf.WRITE_ONLY, agauss_values.nbytes)

	knl_agauss = prg.agauss
	knl_agauss(queue, (p.shape[0],), None, p_g, agauss_values_g)

	cl.enqueue_copy(queue, agauss_values, agauss_values_g)	

	return agauss_values

# Full PPTBF generation
def cl_pptbf(im_size, height_size, tx, ty, zoom, alpha, tt,
	params, num_zero, image_name, image_t_name,
	image_concat_name=None):

	image_np = np.zeros((im_size, im_size), dtype=np.float32)
	image_g = cl.Buffer(ctx, mf.WRITE_ONLY, image_np.nbytes)
	knl_pptbf = prg.pptbf
	
	# Params: jitter, theta, zetax, zetay, amp, larp, normblend,
	...
	# Crop images to the specified height size
	image_np = image_np[:height_size,:]
	...
	if image_concat_name != None:
		output_image = np.stack((image_np, 1.0 - image_np,
		image_np_bin), axis=-1)
		output_image = output_image * 255
		output_image = Image.fromarray(output_image.astype(np.ubyte))
		output_image.save(image_concat_name)
