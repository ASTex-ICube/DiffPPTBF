...
__kernel void agauss(__global float *p,
                    __global __write_only float *agauss_values
                    ) {
  
  // size: number of points in p
  // There are 6 values for each point
  
  int k = get_global_id(0);

  // Note: the seeding is based on the non-jittered points
  // (in the original code, the jittered point is used)
  seeding((uint)(p[6 * k + 0] * 15.0f + 2.0f),
          (uint)(p[6 * k + 1] * 15.0f + 5.0f), 0u);

  // Note: do not assign next() directly to
  // the array values (bug?)
  float ga = next();
  float gb = next();
  float gc = next();
  agauss_values[3 * k + 0] = ga;
  agauss_values[3 * k + 1] = gb;
  agauss_values[3 * k + 2] = gc;
}
...
float procedural_pptbf(float xx, float yy, float tx, float ty, float zoom,
                       float alpha, int tt, float jitter_s,
                       float theta_s, float zetax_s, float zetay_s,
                       float amp_s, float larp, float normblend,
                       float winfeatcorrel, float feataniso_s,
                       float sigcos_s, float deltaorient_s,
                       float wsmooth) {

  float pptbf = 0.0f;

  float sigcos = sigcos_s * 10.0f;
  float deltaorient = deltaorient_s * (-0.1f * M_PI) + (1.0f - deltaorient_s) * (0.1f * M_PI);
  float feataniso = feataniso_s * 5.0f;
  float jitter = (1.0f - jitter_s) * 0.01f + jitter_s * 0.5f;
  float amp = amp_s * 0.1f;
  ...
  float theta = (1.0f - theta_s) * (-0.2f * M_PI) + theta_s * (0.2f * M_PI);
	float zetax = zetay_s;
  float zetay = zetax_s;

  float xx1 = (xx0 * cos(theta) - yy0 * sin(theta));
	float yy1 = (xx0 * sin(theta) + yy0 * cos(theta));
  ...
  //float amp_zoom = 0.25f * amp;
  float amp_zoom = 1.0f * amp;

  float ppx = xx1 + amp_zoom * cnoise2DG(xx1 * zoom * 0.5f + rx, yy1 * zoom * 0.5f) +
        amp_zoom * 0.5f * cnoise2DG(xx1 * zoom + rx, yy1 * zoom) +
        amp_zoom * 0.25f * cnoise2DG(xx1 * zoom * 2.0f + rx, yy1 * zoom * 2.0f) +
        amp_zoom * 0.125f * cnoise2DG(xx1 * zoom * 4.0f + rx, yy1 * zoom * 4.0f);

  float ppy = yy1 + amp_zoom * cnoise2DG(xx1 * zoom * 0.5f, yy1 * zoom * 0.5f + ry) +
        amp_zoom * 0.5f * cnoise2DG(xx1 * zoom, yy1 * zoom + ry) +
        amp_zoom * 0.25f * cnoise2DG(xx1 * zoom * 2.0f, yy1 * zoom * 2.0f + ry) +
        amp_zoom * 0.125f * cnoise2DG(xx1 * zoom * 4.0f, yy1 * zoom * 4.0f + ry);

  float x = 100.0f + (ppx * cos(-alpha) + ppy * sin(-alpha)) * zoom / zetay;
	float y = 100.0f + (-ppx * sin(-alpha) + ppy * cos(-alpha)) * zoom / zetax;
  ...
  for (int k = 0; k < npp; k++) {
...

    {
      ...
      float dd2 = pow(pow(fabs(ddx), 2.0f) + pow(fabs(ddy), 2.0f),
                      1.0f / 2.0f);
...
__kernel void pptbf(const uint size, float tx, float ty, float zoom,
                    float alpha, int tt, 
                    float jitter, float theta, float zetax,
                    float zetay, float amp, float larp,
                    float normblend, float winfeatcorrel,
                    float feataniso, float sigcos,
                    float deltaorient, float wsmooth,
  ...
  image_g[i * size + j] =
      procedural_pptbf(x, y, tx, ty, zoom, alpha, tt,
